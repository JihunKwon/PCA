...
for i=1:numofsubject
    ...
    ave_runA(4*(i-1)+1:4*i) = ave_root(1:4); %A is before drink water
    ave_runB(5*(i-1)+1:5*i) = ave_root(5:9); %B is shortly before drink water
    ave_runC(5*(i-1)+1:5*i) = ave_root(10:14); %C is shortly before drink water
end
...
%Scalar
figure
ave_all_root = [ave_runA ave_runB ave_runC];
g = [zeros(1,length(ave_runA)), ones(1,length(ave_runB)), 2*ones(1,length(ave_runC))];
