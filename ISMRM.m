...
for sub = 1:num_subject
    ocm_runA(1+(sub-1)*5:sub*5) = mean_square_diff(26:30,1+(sub-1)*3); %Before water
    ocm_runB(1+(sub-1)*5:sub*5) = mean_square_diff(26:30,2+(sub-1)*3); %Shortly after water
    ocm_runC(1+(sub-1)*5:sub*5) = mean_square_diff(26:30,3+(sub-1)*3); %10 min after water
end
...
ave_runA_new = ave_runA(1:end/4*3);
ave_runB_new = ave_runB(1:end/4*3);
ave_runC_new = ave_runC(1:end/4*3);
%*****************************************************************
%}
...
[m_runA_norm_sub, m_runB_norm_sub, m_runC_norm_sub, o_runA_norm_sub, o_runB_norm_sub, o_runC_norm_sub] ...
    = Norm_subject(num_subject, ave_runA, ave_runB, ave_runC, ocm_runA, ocm_runB, ocm_runC);
...
if (size(ave_all_ocm,2)==num_subject*15)
